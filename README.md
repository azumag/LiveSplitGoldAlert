...
| ホットキー | 機能 | 備考 |
|-----------|------|------|
| `Ctrl+Alt+B` | ビープ音のON/OFF切り替え | 常に有効 |
| `Ctrl+Alt+D` | デバッグモードのON/OFF | 常に有効 |
| `Ctrl+Alt+X` | スクリプト終了 | 常に有効 |

### デバッグモード時のみ有効なホットキー

デバッグモードをONにすると、以下のテスト用ホットキーが有効になります：
...
**注:**
- ビープ音は実行中に `Ctrl+Alt+B` で切り替えることもできます
- デバッグモードは実行中に `Ctrl+Alt+D` で切り替えることもできます
...
1. LiveSplit Serverが起動しているか確認
   - デバッグモードON（`Ctrl+Alt+D`）にして `Ctrl+Alt+V` でテスト可能
2. ポート番号が `16834` になっているか確認
3. LiveSplitにBest Segmentsデータが存在するか確認
   - 最低1回は完走している必要があります
4. デバッグログを確認
   - デバッグモードON（`Ctrl+Alt+D`）にして `Ctrl+Alt+L` でログを開く
...
3. OBSのホットキー設定を確認
4. ホットキー送信テスト
   - デバッグモードON（`Ctrl+Alt+D`）にして `Ctrl+Alt+H` でテスト可能
